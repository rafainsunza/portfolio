...
const CustomIcon = ({ size, icon }) => {
  const width = size;
  const height = (132 / 161) * size;

  const uid = useId();

  if (icon === "webcomponents") {
    const gradientA = `wc-a-${uid}`;
    const gradientC = `wc-c-${uid}`;

    return (
      <svg width={width} height={height} viewBox="0 0 161 132" xmlns="http://www.w3.org/2000/svg">
        <defs>
          <linearGradient x1="0%" y1="50%" y2="50%" id={gradientA}>
            <stop stopColor="#2A3B8F" offset="0%" />
            <stop stopColor="#29ABE2" offset="100%" />
          </linearGradient>
          <linearGradient x1="100%" y1="50%" x2="0%" y2="50%" id={gradientC}>
